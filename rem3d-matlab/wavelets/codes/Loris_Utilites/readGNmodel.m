function v=readGNmodel(model,dep,fname,eo)
...
% Last edited by Anant Hariharan, 7-28-17
...
% Added line below to check if mit input file works.
defval('dep',rearthkm-6346)
defval('model','ME16')
%defval('fname',fullfile(getenv('IFILES'),...
%			'EARTHMODELS','MONTELLI','PRI5w'))
defval('fname','V1ME16_dvvals.txt')
...
while 1
  ...
  %onedepth=shiftdim(reshape(fscanf(fid,'%f',131073),4,N*N*6),1);
