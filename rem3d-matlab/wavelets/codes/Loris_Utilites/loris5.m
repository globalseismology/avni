function varargout=loris5(wav,N,J,precon,depkm,xver,gnorjr,panel,depthfile)
...
% Identify defaults
defval('depthfile','GN_Depths.txt')
defval('wav','D4')
defval('N',7)
...
else % any other epix format, load all depthss
end  
d = load(depthfile);
defval('depkm',d/1000);
defval('xver',0);
...
if gnorjr==1
  ...
  fname2= sprintf('loris5_GN_%i_%i_%s_%i_%i_%i.mat',...
...
if ndeps>1
  ...
  if gnorjr==1
    ...
    fname= sprintf('loris5_GN_%4.4i_%i_%i_%s_%i_%i.mat',...
