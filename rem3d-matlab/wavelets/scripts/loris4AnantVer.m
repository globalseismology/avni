function loris4AnantVer(depkm,wav,N,J,precon,xver,doneven,actprint,gnorjr,agu)
...
if ~isstr(depkm)
  ...
  for index=1:length(tpercs1)
	...
	cb=colorbar('horiz','FontSize',10,'fontname','Helvetica','position',pos);
...
elseif strcmp(depkm,'demo1')
  ...
  for i=[1 3 6 10 15 19 28 31 46 68 90 129];
    loris4AnantVer(d(i),'D2',[],[],[],[],[],1); 
    loris4AnantVer(d(i),'D4',[],[],[],[],[],1); 
    loris4AnantVer(d(i),'D6',[],[],[],[],[],1); 
  end
elseif strcmp(depkm,'demo2')
  ...
  for i=[10 19 28 46 90];
    disp(' ')
    loris4AnantVer(d(i),'D2',[],[],[],[],1,[],2); 
    disp(' ')
    loris4AnantVer(d(i),'D4',[],[],[],[],1,[],2); 
    disp(' ')
    loris4AnantVer(d(i),'D6',[],[],[],[],1,[],2); 
    disp('---------------------------------------')
  end
  % Ready for tabulating some special values
  % Montelli model
  for i=[10 19 28 46 90];
    disp(' ')
    loris4AnantVer(d(i),'D2',[],[],[],[],1,[],1); 
    disp(' ')
    loris4AnantVer(d(i),'D4',[],[],[],[],1,[],1); 
    disp(' ')
    loris4AnantVer(d(i),'D6',[],[],[],[],1,[],1); 
    disp('---------------------------------------')
  end
end
