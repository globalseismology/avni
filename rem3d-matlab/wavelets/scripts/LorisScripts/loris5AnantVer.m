function varargout=loris5(wav,N,J,precon,depkm,xver,gnorjr,panel,modelname)
...
% loris5AnantVer('D4',7,4,[1 1],[],0,2) % Paper figure B
%
% Last modified by fjsimons-at-alum.mit.edu, 2/28/2011

% Identify defaults
defval('wav','D4')
defval('modelname','ME16')
defval('N',7)
...
%%%%unsure: Anant 6-26-17
d = load('GN_Depths.txt');
%%%%
...
if gnorjr==1
  ...
  fname2= sprintf(['loris5_' modelname 'GN_%i_%i_%s_%i_%i_%i.mat'],...
			 N,J,wav,precon(1),precon(2),panel);
  %fname2 = '/home/anant/Dropbox/Analyses/wavelets/test.mat'
elseif gnorjr==2
  % And now define what will come out of the analysis at the end
  fname2= sprintf('loris5_JR_%i_%i_%s_%i_%i_%i.mat',...
			 N,J,wav,precon(1),precon(2),panel);
  %fname2 = '/home/anant/Dropbox/Analyses/wavelets/test.mat'
...
if ndeps>1
  if exist(fname2,'file')==2
    ...
    for index=1:ndeps
      ...
      [vw,depchk,vstats]=...
	  loris5AnantVer(wav,N,J,precon,depkm(index),xver,gnorjr,panel);
  ...
  if gnorjr==1    
    ...
    fname= sprintf(['loris5_' modelname 'GN_%4.4i_%i_%i_%s_%i_%i.mat'],...
			   round(depkm),N,J,wav,precon(1),precon(2));
    %fname = '/home/anant/Dropbox/Analyses/wavelets/test2.mat'

  elseif gnorjr==2
    % And now define what will come out of the results at the end
    fname= sprintf('loris5_JR_%4.4i_%i_%i_%s_%i_%i.mat',...
			   round(depkm),N,J,wav,precon(1),precon(2));
    %fname = '/home/anant/Dropbox/Analyses/wavelets/test2.mat'
  end
