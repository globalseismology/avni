...
vercof,dvercof = rem3d.tools.bases.eval_polynomial(rnorm-150.,radius_range=[rnorm-200.,rnorm-100.] ,rnorm=rnorm,types=['TOP','BOTTOM'])
top= bottom = 3.45 #m/s
coeff = np.array([[top,bottom]])
assert np.isclose(sum(coeff*vercof),top), 'evaluation of TOP/BOTTOM should give constant value anywhere in the domain'