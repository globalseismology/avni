...
def main():
    ...
    data_vars['shold_convert'] = (('latitude', 'longitude'),(sholdcof[:,index].reshape(len(latitude),len(longitude))).toarray())

    # pyshtools with ortho norm and condon-shortley pahse-Same as shold
    cilm = np.zeros((2,lmaxhor+1,lmaxhor+1))
    if CS.lower()=='c' :cilm[0,L,M]=1
    if CS.lower()=='s' : cilm[1,L,M]=1
    topo = pyshtools.expand.MakeGridDH(cilm, sampling=2,norm=4,lmax=(180./spacing-2)/2,csphase=-1)
    data_vars['shtools_orthoCS'] = (('latitude', 'longitude'),topo)
    ...
    data_vars['shtools_convert'] = (('latitude', 'longitude'),topo)

    # make a composite array
    ...
    diff1 = xrdata['shold_convert']-xrdata['ylm']
    diff2 = xrdata['shold']-xrdata['shtools_orthoCS']
    frac = xrdata['shold']/xrdata['ylm']
    ...
    if arg.output:
        fig, axes = plt.subplots(ncols=2,nrows=4,sharex='col')
        xrdata['ylm'].plot(ax=axes[0,0])
        xrdata['shold_convert'].plot(ax=axes[1,0])
        xrdata['shtools_convert'].plot(ax=axes[2,0])
        (diff1).plot(ax=axes[3,0])
        axes[3,0].set_title('shold_convert-shtools_orthoCS')

        xrdata['shold'].plot(ax=axes[0,1])
        xrdata['shtools_orthoCS'].plot(ax=axes[1,1])
        (diff2).plot(ax=axes[2,1])
        axes[2,1].set_title('shold-shtools_orthoCS')
        (frac).plot(ax=axes[3,1])
        axes[3,1].set_title('shold/ylm')
