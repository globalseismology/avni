...
#####Sub-Sub Reading Subroutines
def Read_GridMat_Files(Database_Path,N,Jmax,EulerConfig,ModelName,Sphere_Param):
    """ Reads maps of Grids, which contain describe a point's lat, lon, face, etc.
    """
    ####More Setup for File Names
    ##Naming Conventions
...
def Read_Bases_Mat_Files(Database_Path,N,Jmax,WaveletBasisFn,Preconditioning):
    """ Reads basis maps for wavelets and wavelet coefficients. 
    """
    ##Naming Conventions
    ...
    return(DVBasisName,WaveletCoeffBasisName,WaveletBasis,WaveletCoeffBasis)


def Read_Model(Database_Path,N,Jmax,EulerConfig,ModelName,Sphere_Param):
    """
    Reads an interpolated tomographic model generated using REM3D-MATLAB, outputs
    wavelet coeffs, interpolated depths, velocity anomalies. 
    """
    ##Naming Conventions
    VeloName = Sphere_Param + '_' + ModelName + '.N' + str(N) + '.Jmax' + str(Jmax) +  \
                  '.EulerConfig' + str(EulerConfig) + '.mat'
    ...
    return(Vvals,wvcoeffs,Depths,Depth_List,ModelFile,VeloName)


def Read_Mat_Files_Ver2 (Database_Path,N,Jmax,WaveletBasisFn,eo,EulerConfig,ModelName,Sphere_Param,Preconditioning):
    """ This takes in a set of parameters that specify the .mat files in the rem3d/files/ folder, and reads in 
    the relevant files using these parameters. Then outputs the files and some relevant variables from them. 
    This function is thus essential to do anything!
    ...
    #    Preconditioning = 'Precon'  #Precoff or Precon
    """
    (GridName,Grids,lat,lon) = Read_GridMat_Files(Database_Path,N,Jmax,EulerConfig,ModelName,Sphere_Param)
    (DVBasisName,WaveletCoeffBasisName,WaveletBasis,WaveletCoeffBasis) = Read_Bases_Mat_Files(Database_Path,N,Jmax,WaveletBasisFn,Preconditioning)
    (Vvals,wvcoeffs,Depths,Depth_List,ModelFile,VeloName) = Read_Model(Database_Path,N,Jmax,EulerConfig,ModelName,Sphere_Param)
    return(DVBasisName,WaveletCoeffBasisName,GridName,VeloName,WaveletBasis,WaveletCoeffBasis,Grids,ModelFile,Vvals,wvcoeffs,Depth_List)
