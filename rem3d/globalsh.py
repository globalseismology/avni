...
import sys
############ input PM modules ######
from drawmapsection3D import get_fullpath
...
def rdswpsh(filename,maxl=360):
    ...
    if (not os.path.isfile(filename)): sys.exit("Filename ("+filename+") does not exist")
...
def calcshpar2(shmatrix):
    ...
    for ii in np.arange(len(shmatrix['l'])):
        if shmatrix['m'][ii] == 0:
            ...
            icount = icount + 2
    if icount != (lmod+1)**2: sys.exit("Error: icount != (lmod+1)**2")       
...
#     fly
    ...
    for l in np.arange(0,lmod+1):
        ...
        for m in np.arange(0,l+1):
            if m == 0:
                coeff=shmod[i]/np.sqrt(4.*pi)
                power=power+coeff**2
                i=i+1
            else:
                coeff=np.sqrt(2.)*0.5*shmod[i]/np.sqrt(4.*pi)
                power=power+coeff**2
                coeff=-np.sqrt(2.)*0.5*shmod[i+1]/np.sqrt(4.*pi)
                power=power+coeff**2
                i=i+2
        powerarr[l]=power
    powerall=0.
