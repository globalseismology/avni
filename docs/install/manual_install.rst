...
AVNI requires Python version |min_python_version| or higher. If you
...
Installing a minimal AVNI with EEGLAB I/O support
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
If you plan to use AVNI's functions that require **HDF5 I/O** (this
