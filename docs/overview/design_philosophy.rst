...
return `matplotlib`_ :class:`~matplotlib.figure.Figure` objects, and the 3D
plotting functions return :class:`avni.viz.Figure3D` classes with a ``.plotter``
attribute pointing to :class:`avni.Plotter` instances, so you can customize your
...
Because neuroimaging datasets can be quite large, AVNI tries very hard to avoid
...
memory-saver when applying a finished script to dozens of subjects' worth of
data.
